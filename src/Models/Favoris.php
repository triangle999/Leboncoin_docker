...
class Favoris
{
    ...
    public function findByUser(int $userId): array
    {
        ...
        try {
            ...
            $sql = 'SELECT `f_id`, `user_id`, `annonce_id`, `a_id`, `a_title`, `a_description`, `a_price`, `a_picture`, `a_publication`, `u_id` FROM `favoris` NATURAL JOIN `users` NATURAL JOIN `annonces` WHERE u_id = :userId';
            // on prépare la requête avant de l'executer 
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(':userId', $userId, PDO::PARAM_STR);
